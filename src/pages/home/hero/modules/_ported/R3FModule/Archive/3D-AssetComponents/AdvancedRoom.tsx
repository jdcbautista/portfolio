...
interface WallObjects {
  ...
  interface AdvancedRoomSectionWallProps {
    ...
    wallHeightOverride: number | null;              // If we want to override section wall height
...
interface AdvancedRoomProps {
  ...
  // AdvancedRoom will be passed AdvancedRoomProps, which will have a list of Sections to iterate through.
  // Each entry within Sections will contain a list of walls to iterate through

const AdvancedRoom = ({ ...props }: AdvancedRoomProps) => {
  // const [ref] = useBox(() => ({
  //   type: 'Static',
  //   mass: 1,
  //   onCollide: (e) => {
  ...
  // 
  // function ConsoleLogFunction({...props}){
    ...
    <group>
        ...
        {props.sections.map((section, sIndex) => (<group key={sIndex}>
          {section.walls.map((wall, wIndex) => (
            <group key={wIndex}>
            <AdvancedWall key={wIndex} intSegments={wall.intSegments} extSegments={wall.extSegments} interiorWallObjects={wall.interiorWallObjects} exteriorWallObjects={wall.exteriorWallObjects} className={wall.type} wallHeight={section.sectionHeight}
